  ...
  return (
    <Section className="bg-dark-olive-700" sectionId={SectionId.Portfolio}>
      <div className="flex flex-col gap-y-8">
        <h2 className="self-center text-xl font-bold text-white">Check out some of my work (da aggiungere ancora i lavori) </h2>
    ...
    <>
      ...
      >
        <div className="relative h-full w-full p-4 flex flex-col justify-center">
          ...
          <ArrowTopRightOnSquareIcon aria-label={`Vai al progetto ${title}`} className="absolute bottom-2 right-2 h-4 w-4 shrink-0 text-white sm:bottom-3 sm:right-3" />
        </div>
      </a>
      {mobile && showOverlay && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur flex items-center justify-center" onClick={handleCloseOverlay}>
          <div className="relative w-11/12 max-w-lg bg-neutral-800 rounded-lg p-6 overflow-y-auto overscroll-contain">
            ...
            <Link href={url} target="_blank" rel="noopener noreferrer" className='mt-4 w-fit mx-auto bg-orange-500 rounded-md py-2 px-4 text-white hover:bg-orange-600'>Vai al Progetto</Link>
          </div>
        </div>
      )}
    </>
